...
namespace MoreMountains.FeedbacksForThirdParty
{
    ...
    public class MMF_Haptics : MMF_Feedback
    {
        ...
        public enum HapticMethods { NativePreset, Transient, Continuous, AdvancedPattern, Stop, AdvancedTransient }
        /// the timescale to operate on
        public enum Timescales { ScaledTime, UnscaledTime }

        // NATIVE PRESET -----------------------------------------------------------------------------------------------------
        [MMFInspectorGroup("Haptics", true, 37)]
        ...
        // RUMBLE -------------------------------------------------------------------------------------------------------------
        [MMFInspectorGroup("Rumble", true, 33)]
            ...
            switch (HapticMethod)
            {
                case HapticMethods.AdvancedPattern:
                    ...
                    MMVibrationManager.AdvancedHapticPattern(APVibrateIOS, iOSString, APVibrateAndroid, androidPattern, androidAmplitude, 
                        AndroidRepeat, APVibrateAndroidIfNoSupport, APRumble, 
                                                                ...
                                                                OldIOSFallback, Owner, ControllerID);
                    break;

                case HapticMethods.Continuous:
                    Owner.StartCoroutine(ContinuousHapticsCoroutine());
                    break;

                case HapticMethods.NativePreset:
                    MMVibrationManager.Haptic(HapticType, false, AllowRumble, Owner, ControllerID);
                    break;

                case HapticMethods.Transient:
                    MMVibrationManager.TransientHaptic(TransientIntensity, TransientSharpness, AllowRumble, Owner, ControllerID);
                    break;
                ...
                case HapticMethods.AdvancedTransient:
                    MMVibrationManager.TransientHaptic(ATVibrateIOS, ATIOSIntensity, ATIOSSharpness, ATVibrateAndroid,
                        ...
                        ATRumbleIntensity, ATRumbleSharpness, ATRumbleControllerID, Owner);
                    break;
        ...
        {
            _continuousStartedAt = (Timing.TimescaleMode == TimescaleModes.Scaled) ? Time.time : Time.unscaledTime;
            _continuousPlaying = true;
            float elapsedTime = ComputeElapsedTime();

            MMVibrationManager.ContinuousHaptic(InitialContinuousIntensity, InitialContinuousSharpness, ContinuousDuration, HapticTypes.Success, Owner);
        ...
        {
            float elapsedTime = (Timing.TimescaleMode == TimescaleModes.Scaled) ? Time.time - _continuousStartedAt : Time.unscaledTime - _continuousStartedAt;
            return elapsedTime;
        }
