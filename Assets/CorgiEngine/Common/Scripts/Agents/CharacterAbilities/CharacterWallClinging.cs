...
{	
	...
	public class CharacterWallClinging : CharacterAbility 
	{
        ...
        /// if this is set to true, the player will remain in a wallclinging state for a brief time even if the player inputs a direction away from the wall
        [Tooltip("if this is set to true, the player will remain in a wallclinging state for a brief time even if the player inputs a direction away from the wall")]
        public bool LockToWall = false;
        protected int LockCounter = 0;
        public bool CountingDown = false;

        protected bool ExitStarted = false;
        ...
        private float movingWallX = 0f;
        private float movingWallY = 0f;
	    ...
	    protected virtual void WallClinging()
		{
            ...
            else
            {
                ...
                if (TestForWall() && ((_character.IsFacingRight && _horizontalInput >= _inputManager.Threshold.x) || (!_character.IsFacingRight && _horizontalInput <= -_inputManager.Threshold.x)))
                {
                    Debug.Log("ready to start clinging");
                    EnterWallClinging();
                }
            }            
		}
        ...
        protected virtual void EnterWallClinging()
        {
            ...
            if (_raycast.point.x > transform.position.x)
            {
                Vector2 newPosition = new Vector2(_raycast.point.x - 0.08f - _controller.Width() / 2, transform.position.y);
            ...
            movingWallX = 0f;
            movingWallY = 0f;
            //previousMaxSpeed = _controller.
        }

        protected virtual void WhileClinging()
        {
            if (_movement.CurrentState == CharacterStates.MovementStates.WallClinging)
            {
                MovingPlatform currentMovingPlatform = null;
                if (_controller.CurrentWallCollider != null)
                {
                    currentMovingPlatform = _controller.CurrentWallCollider.gameObject.GetComponent<MovingPlatform>();
                } else if (_raycast.collider != null)
                {
                    currentMovingPlatform = _raycast.collider.gameObject.GetComponent<MovingPlatform>();
                }
                if (currentMovingPlatform != null)
                {
                    movingWallX = currentMovingPlatform.CurrentSpeed.x;
                    movingWallY = currentMovingPlatform.CurrentSpeed.y;
                    _controller.SetUncappedForce(currentMovingPlatform.CurrentSpeed);
                }
		...
		protected virtual void ExitWallClinging()
		{
			if (_movement.CurrentState == CharacterStates.MovementStates.WallClinging)
			{
                ...
                {
                    // we cast our ray 
                    RaycastHit2D hit = MMDebug.RayCast(raycastOrigin, raycastDirection, 0.5f, _controller.PlatformMask & ~(_controller.OneWayPlatformMask | _controller.MovingOneWayPlatformMask), Color.red, _controller.Parameters.DrawRaycastsGizmos);
        ...
        protected virtual void BeginExitCountdown()
        {
            if (!CountingDown)
            {
                //after X frames, leave the wall. Used to provide brief buffer when inputting walljumps
                LockCounter = 65;
                CountingDown = true;
            }
        }
