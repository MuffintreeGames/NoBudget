...
{	
	...
	public class CharacterHandleWeapon : CharacterAbility 
	{
		...
		/// if this is true the character will automatically fire whenever possible
		[Tooltip("if this is true the character will automatically fire whenever possible")]
		public bool AutoFire = false;
		// time until autofire should begin
		[MMCondition("AutoFire", true)]
		[Tooltip("time until autofire should begin")]
		...
		protected bool firingCountdownOn;
		...
		public virtual void Setup()
        {
			...
			Debug.Log("Animator is " + CharacterAnimator);
			...
			if (AutoFire)
            {
				delayTime = autofireDelay;
				sprite = this.GetComponent<SpriteRenderer>();
				originalColor = sprite.color;
            }
			animationTime = 0f;
			firingCountdownOn = false;
		}
		...
		public override void ProcessAbility()
		{
			...
			if (AutoFire)
			{
				...
				else
				{
					...
					if (CurrentWeapon._delayBeforeUseCounter > 0f)
                    {
						timeUntilNextShot = CurrentWeapon._delayBeforeUseCounter;
					} else
                    {
						timeUntilNextShot = CurrentWeapon._delayBetweenUsesCounter;
                    }

					percentage = timeUntilNextShot / (CurrentWeapon.TimeBetweenUses / 2);
					percentage = Mathf.Max(0, percentage);
					percentage = Mathf.Min(1, percentage);
					Color newColor = Color.red + (percentage * (originalColor - Color.red));
					sprite.color = newColor;
					ShootStart();
				}
			}
			else if (firingCountdownOn)
			{
				
				if (animationTime <= 0f)
				{
					Debug.Log("Stop fire");
					CharacterAnimator.SetBool("Firing", false);
					firingCountdownOn = false;
				}
				else
				{
		...
		public virtual void ShootStart()
		{
			...
			CharacterAnimator.SetBool("Firing", true);
