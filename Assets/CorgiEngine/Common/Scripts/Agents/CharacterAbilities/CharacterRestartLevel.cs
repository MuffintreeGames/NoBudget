...
{	
	...
	public class CharacterRestartLevel : CharacterAbility 
	{
		...
		public enum RestartModes { KillPlayer, GoToScene, CurrentCheckpoint, FirstCheckpoint, LastCheckpoint, LevelManager }
        ...
        {
			if (_condition.CurrentState == CharacterStates.CharacterConditions.Paused)
            {
				return;
            }
	        ...
	        switch (RestartMode)
	        {
		        ...
		        case RestartModes.LastCheckpoint:
			        ...
			        break;
				case RestartModes.LevelManager:
					LevelManager.Instance.GotoLevel(SceneManager.GetActiveScene().name, true, false);
					break;
			}
        }
	}
