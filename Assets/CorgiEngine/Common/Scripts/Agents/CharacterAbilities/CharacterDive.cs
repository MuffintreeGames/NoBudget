...
{	
	/// Add this class to a Character and it'll be able to dive by pressing the slide button in the air, pounding the ground in the process.
	...
	{	
		...
		public override string HelpBoxText() { return "This component allows your character to dive (by pressing the slide button while in the air). Here you can define how much the camera should shake on impact, and how fast the dive should be."; }
		...
		protected float speedAtStart = 0f;
		...
		protected virtual void InitiateDive()
		{
			...
			speedAtStart = _controller.Speed.x;
			// we start the dive coroutine
			StartCoroutine(Dive());
		}
	    ...
	    protected virtual IEnumerator Dive()
		{
			...
			while (!_controller.State.IsGrounded)
			{
				...
				_controller.SetHorizontalForce(speedAtStart / 2);
