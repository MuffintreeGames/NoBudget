...
{	
	...
	{		
        ...
        /// if this is true, part of the force applied by a dash will be maintained relative to the character's current speed
        [Tooltip("if this is true, part of the force applied by a dash will be maintained relative to the character's current speed")]
        public bool VariableSpeedExit = true;

        /// the minimum speed of the dash exit
        [Tooltip("the minimum speed of the dash exit")]
        [MMCondition("VariableSpeedExit", true)]
        public float ExitMinSpeed = 0f;

        /// the maximum speed of the dash exit
        [Tooltip("the maximum speed of the dash exit")]
        [MMCondition("VariableSpeedExit", true)]
        public float ExitMaxSpeed = 100f;
        ...
        [Header("Super Slide")]
        /// if this is true, the character can perform a 'super slide' by dashing while sliding
        [Tooltip("if this is true, the character can perform a 'super slide' by dashing while sliding")]
        public bool SuperSlideEnabled = true;
        ...
        // dash icon on GUI
        protected Image dashIcon;
        protected Text iconTimer;
        ...
        protected override void Initialization()
        {
            ...
            dashIcon = GameObject.Find("UICamera/Canvas/AbilityIcons/Dash").GetComponent<Image>();
            iconTimer = GameObject.Find("UICamera/Canvas/AbilityIcons/Dash/Timer").GetComponent<Text>();
        }
		...
		public override void ProcessAbility()
		{
            ...
            if (_inCooldown)
            {
                ...
                float remainingCooldown =  _cooldownTimeStamp - Time.time;
                iconTimer.text = remainingCooldown.ToString("F1");
            } else
            {
        ...
        {
            if (SuccessiveDashesLeft < 1)
            {
                Debug.Log(SuccessiveDashesLeft);
            }
        ...
        {
            Debug.Log("dash");
            if (_movement.CurrentState == CharacterStates.MovementStates.Sliding)   //if we're currently sliding, attempt a super-slide
            {
                if (!SuperSlideEnabled)
                    return;
            }
            ...
            speedBeforeDash = _controller.Speed.x;
            if (LimitedDashes)
            {
		...
		protected virtual IEnumerator Dash()
        {
                ...
                && _movement.CurrentState == CharacterStates.MovementStates.Dashing)
            {
				...
				{
                    _controller.GravityActive(false);
                    _controller.SetUncappedForce(_dashDirection * speedOfDash);
				}
				yield return null;
            }
        ...
        public virtual void StopDash()
        {
            ...
            {
                _controller.SetForce(Vector2.zero);
            } else if (VariableSpeedExit)
            {
                Vector2 exitVector = new Vector2((ExitMaxSpeed - ExitMinSpeed) * (speedBeforeDash / _controller.Parameters.MaxVelocity.x), 0);
                _controller.SetForce(exitVector);
            }
