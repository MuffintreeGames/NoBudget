...
{	
	...
	public class CorgiController : MonoBehaviour 
	{
		...
		/// The layer mask the mid height one way platforms are on
		[Tooltip("The layer mask the mid height one way platforms are on")]
		...
		protected bool speedLimitsEnabled = true;
		...
		/// <summary>
        ///  apply a force which can break the normal caps on velocity
        /// </summary>
        /// <param name="force"></param>
		public virtual void SetUncappedForce (Vector2 force)
        {
			_speed = force;
			_externalForce = force;
		}
			...
			{	
				...
				if (_sideHitsStorage[i].distance > 0)
				{
					...
					if (hitAngle > Parameters.MaximumSlopeAngle)
					{
						...
						MovingPlatform movingWall = CurrentWallCollider.GetComponent<MovingPlatform>();
						if (movingWall != null)
						{

							float wallSpeed = movingWall.CurrentSpeed.x;


							if (raysDirection * wallSpeed < 0f)	//getting pushed by a moving wall
							{
								if (Mathf.Abs(_speed.x) < Mathf.Abs(wallSpeed))
								{
									_newPosition.x = wallSpeed * DeltaTime;
									if (raysDirection < 0)
									{
										_speed.x = Mathf.Max(_speed.x, wallSpeed);
									}
									else
									{
										_speed.x = Mathf.Min(_speed.x, wallSpeed);
									}
									_shouldComputeNewSpeed = true;
								}
							}
		...
		public virtual void DisableSpeedLimits()
        {
			speedLimitsEnabled = false;
        }

		public virtual void EnableSpeedLimits()
        {
			speedLimitsEnabled = false;
        }
