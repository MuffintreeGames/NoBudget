...
{	
	...
	public class CameraController : MonoBehaviour, MMEventListener<CorgiEngineEvent>, MMEventListener<MMCameraEvent>
    {
		...
		/// How fast the camera moves in dramatic mode
		[Tooltip("How fast the camera moves in dramatic mode")]
		public float DramaticCameraSpeed = 0.1f;
		...
		/// the minimum camera zoom when in dramatic mode
		[Range(1, 20)]
		[Tooltip("the minimum camera zoom when in dramatic mode")]
		public float DramaticMinimumZoom = 6.5f;
		/// the maximum camera zoom when in dramatic mode
		[Range(1, 20)]
		[Tooltip("the maximum camera zoom when in dramatic mode")]
		public float DramaticMaximumZoom = 6.5f;
		/// the speed at which the camera zooms	in dramatic mode
		[Tooltip("the speed at which the camera zooms in dramatic mode")]
		public float DramaticZoomSpeed = 1f;
		...
		protected bool dramaticZoom = false;
		...
		protected virtual void Initialization ()
		{
			...
			// if we have a level manager, we grab our level bounds and optionally our no going back object
		...
		protected virtual void FollowPlayer()
		{
			...
			{
				_lookAheadPos = Vector3.MoveTowards(_lookAheadPos, Vector3.zero, Time.unscaledDeltaTime * ResetSpeed);	
			}
			...
			if (dramaticZoom || _currentZoom < (MinimumZoom - 1f))
			{
				newCameraPosition = Vector3.SmoothDamp(transform.position, aheadTargetPos, ref _currentVelocity, DramaticCameraSpeed, Mathf.Infinity, Time.unscaledDeltaTime);
			} else
            {
				newCameraPosition = Vector3.SmoothDamp(transform.position, aheadTargetPos, ref _currentVelocity, CameraSpeed, Mathf.Infinity, Time.unscaledDeltaTime);
			}
		...
		protected virtual void Zoom()
		{
			...
			if (dramaticZoom)
			{
				_currentZoom = Mathf.SmoothDamp(_currentZoom, (characterSpeed / 10) * (DramaticMaximumZoom - DramaticMinimumZoom) + DramaticMinimumZoom, ref currentVelocity, DramaticZoomSpeed, Mathf.Infinity, Time.unscaledDeltaTime);
			}
			else if (_currentZoom < (MinimumZoom - 1f))
			{
				_currentZoom = Mathf.SmoothDamp(_currentZoom, (characterSpeed / 10) * (MaximumZoom - MinimumZoom) + MinimumZoom, ref currentVelocity, DramaticZoomSpeed);
			} else
            {
		...
		protected virtual void DramaticZoom()
        {
			dramaticZoom = true;
		}

		protected virtual void DramaticUnzoom()
		{
			dramaticZoom = false;
		}
