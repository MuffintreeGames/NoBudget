...
{	
	...
	/// </summary>
	public class GUIManager : MMSingleton<GUIManager>, MMEventListener<LevelNameEvent>, MMEventListener<LevelNameSplashEvent>, MMEventListener<ControlsModeEvent>, MMEventListener<EnemyCountEvent>
	{
        [Header("Bindings")]
		...
		private GameObject LevelSplashName;
		private GameObject LevelSplashPrompt;
		...
		protected override void Awake()
		{
			...
			LevelSplashName = GameObject.Find("LevelName");
			LevelSplashName.GetComponent<Text>().enabled = false;
			LevelSplashPrompt = GameObject.Find("JumpPrompt");
			LevelSplashPrompt.GetComponent<Text>().enabled = false;
		}
		...
		protected virtual void Update()
        {
			...
			else if (countdownActive)
            {
                ...
                {
					LevelSplashName.GetComponent<Text>().text = ((countdown * 3) + 0.5f).ToString("F0"); 
                }
            }
        }
        ...
        {
			LevelSplashPrompt.SetActive(false);
			awaitingStart = false;
        ...
        {
			LevelSplashName.SetActive(false);
		...
		public virtual void UpdateEnemyCount()
        {
		...
		public virtual void OnMMEvent(LevelNameSplashEvent levelNameSplashEvent)
		{
			awaitingStart = true;
			countdownActive = false;
			countdown = 3f;
			Text levelName = LevelSplashName.GetComponent<Text>();
			levelName.enabled = true;
			levelName.text = levelNameSplashEvent.LevelName;
			LevelSplashPrompt.GetComponent<Text>().enabled = true;
		}
		...
		public virtual void OnMMEvent(EnemyCountEvent enemyCountEvent)
        {
			enemyCount = enemyCountEvent.EnemyCount;
			UpdateEnemyCount();
        }
        ...
        protected virtual void OnEnable()
        {
			...
			this.MMEventStartListening<LevelNameSplashEvent>();
			this.MMEventStartListening<ControlsModeEvent>();
			this.MMEventStartListening<EnemyCountEvent>();
		}
        ...
        protected virtual void OnDisable()
        {
			...
			this.MMEventStopListening<LevelNameSplashEvent>();
			this.MMEventStopListening<ControlsModeEvent>();
			this.MMEventStopListening<EnemyCountEvent>();
		}
    }
}