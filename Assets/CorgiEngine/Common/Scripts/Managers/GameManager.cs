...
namespace MoreMountains.CorgiEngine
{
								...
								MMEventListener<CorgiEnginePointsEvent>
	{
		...
		/// the feedbacks to use when pausing/unpausing
		[Tooltip("the feedbacks to use when pausing/unpausing")]
		public MMFeedbacks PauseFeedbacks;
		...
		protected bool _inQTE = false;
		protected bool _inSplash = false;
		...
		/// </summary>
		public virtual void Pause(PauseMethods pauseMethod = PauseMethods.PauseMenu, bool pauseSound = true, bool muffleMusic = true)
		{
			Debug.Log("Pause method: " + pauseMethod);
			...
			if (Time.timeScale>0.0f)
            {
				...
				if (muffleMusic)
				{
					MMSoundManagerTrackEvent.Trigger(MMSoundManagerTrackEventTypes.SetVolumeTrack, MMSoundManager.MMSoundManagerTracks.Music, 0.3f);
				}
	    ...
	    /// Unpauses the game
	    /// </summary>
		public virtual void UnPause(PauseMethods pauseMethod = PauseMethods.PauseMenu, bool pauseSound = true)
	    {
			...
			MMSoundManagerTrackEvent.Trigger(MMSoundManagerTrackEventTypes.SetVolumeTrack, MMSoundManager.MMSoundManagerTracks.Music, 1f);
		...
		public virtual void OnMMEvent(CorgiEngineEvent engineEvent)
		{
			switch (engineEvent.EventType)
			{
				...
				case CorgiEngineEventTypes.UnPause:
					...
					break;
				case CorgiEngineEventTypes.UnPauseSilent:
					if (!_inQTE && !_inSplash)
					{
						UnPause(PauseMethods.PauseMenu, false);
					}
					break;
				case CorgiEngineEventTypes.BeginQTE:
					if (!_inSplash)
					{
						Pause(PauseMethods.NoPauseMenu, false, false);
						_inQTE = true;
					}
					break;
				case CorgiEngineEventTypes.EndQTE:
					if (_inQTE)
					{
						...
						if (!_inSplash)
                        {
							UnPause(PauseMethods.NoPauseMenu, false);
						}
					}
					break;
				case CorgiEngineEventTypes.StartSplash:
					Pause(PauseMethods.NoPauseMenu, false);
					_inSplash = true;
					break;
				case CorgiEngineEventTypes.EndSplash:
					if (_inSplash)
					{
						UnPause(PauseMethods.NoPauseMenu, false);
						_inSplash = false;
