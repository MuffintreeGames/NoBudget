...
namespace MoreMountains.CorgiEngine
{
	...
	public struct LevelNameSplashEvent
	{
		public string LevelName;

		/// <summary>
		/// Initializes a LevelNameSplashEvent
		/// </summary>
		/// <param name="levelName"></param>
		public LevelNameSplashEvent(string levelName)
		{
			LevelName = levelName;
		}

		static LevelNameSplashEvent e;
		public static void Trigger(string levelName)
		{
			e.LevelName = levelName;
			MMEventManager.TriggerEvent(e);
	...
	public struct TutorialPopupEvent
	{
		...
		public TutorialPopupEvent(string tutorialName, string tutorialText, VideoClip tutorialVideo)
		{
		...
		static TutorialPopupEvent e;
		public static void Trigger(string tutorialName, string tutorialText, VideoClip tutorialVideo)
		{
			e.TutorialName = tutorialName;
			e.TutorialText = tutorialText;
			e.TutorialVideo = tutorialVideo;
			MMEventManager.TriggerEvent(e);
		}
	}

	public struct CloseTutorialEvent
	{
		public bool Close;

		public CloseTutorialEvent(bool close)
		{
			Close = close;
		}

		static CloseTutorialEvent e;
		public static void Trigger(bool close)
		{
			e.Close = close;
			MMEventManager.TriggerEvent(e);
		}
	...
	public struct EnemyCountEvent
	{
		public int EnemyCount;

		/// <summary>
		/// Initializes a EnemyCountEvent
		/// </summary>
		/// <param name="enemyCount"></param>
		public EnemyCountEvent(int enemyCount)
		{
			EnemyCount = enemyCount;
		}

		static EnemyCountEvent e;
		public static void Trigger(int enemyCount)
		{
	...
	public struct ExitUnlockedEvent
	{
		public bool ExitUnlocked;
		/// <summary>
		/// Initializes a ExitUnlockedEvent
		/// </summary>
		public ExitUnlockedEvent(bool unlocked) {
			ExitUnlocked = unlocked;
		}

		static ExitUnlockedEvent e;
		public static void Trigger(bool unlocked)
		{
			e.ExitUnlocked = unlocked;
			MMEventManager.TriggerEvent(e);
		}
	}
	...
	public struct RestartLevelEvent
	{
		public bool Restart;
		/// <summary>
		/// Initializes a RestartLevelEvent
		/// </summary>
		/// <param name="enemyCount"></param>
		public RestartLevelEvent(bool restart)
		{
			Restart = restart;
		}

		static RestartLevelEvent e;
		public static void Trigger(bool restart)
		{
			e.Restart = restart;
			MMEventManager.TriggerEvent(e);
		}
	...
	public struct TimeUpEvent
	{
		...
		/// <summary>
		/// Initializes a TimeUpEvent
		/// </summary>
		/// <param name="enemyCount"></param>
		public TimeUpEvent(bool killPlayer)
		{
			KillPlayer = killPlayer;
		}

		static TimeUpEvent e;
		public static void Trigger(bool killPlayer)
		{
			e.KillPlayer = killPlayer;
			MMEventManager.TriggerEvent(e);
		}
	}
	...
	public class LevelManager : MMSingleton<LevelManager>, MMEventListener<MMCharacterEvent>, MMEventListener<CloseTutorialEvent>
	{
		...
		/// the level to send a player to after death
		[Tooltip("the level to send a player to after death")]
		...
		///  if this is true, reload the level when this level restarts
		[Tooltip("if this is true, reload the level when this level restarts")]
		public bool ReloadOnRestart = true;
		///  if this is true, trigger a splash screen when the level starts
		[Tooltip("if this is true, trigger a splash screen when the level starts")]
		public bool SplashStart = false;
		///  if this is true, trigger a tutorial popup when the level starts
		[Tooltip("if this is true, trigger a tutorial popup when the level starts")]
		public bool Tutorial = false;
		///  name of the tutorial popup
		[Tooltip("name of the tutorial popup")]
		[MMCondition("Tutorial", true)]
		public string TutorialName;
		/// <summary>
		///  body of the tutorial popup
		/// </summary>
		[Tooltip("body of the tutorial popup")]
		[MMCondition("Tutorial", true)]
		public string TutorialMessage;
		/// <summary>
		///  tutorial video played in the popup
		/// </summary>
		[Tooltip("tutorial video played in the popup")]
		[MMCondition("Tutorial", true)]
		public VideoClip TutorialVideo;
		...
		[Tooltip("the number of enemies in the level")]
		public int EnemyCount;
		...
		protected bool shownTutorial = false;
		protected bool shownSplash = false;
		...
		public virtual void Start()
		{
			...
			Timer = this.GetComponent<LevelTimer>();
			if (Timer != null)
			{
        ...
        {
			if (Tutorial && !shownTutorial)
			{
				shownTutorial = true;
				bool seenTutorial = saver.GetSeenTutorial(TutorialName);
				if (!seenTutorial)
				{
					inTutorial = true;
					TutorialPopupEvent.Trigger(TutorialName, TutorialMessage, TutorialVideo);
					if (!GameManager.Instance.Paused)
					{
						CorgiEngineEvent.Trigger(CorgiEngineEventTypes.StartSplash);
						ToggleCharacterPause();
					}
				}
			}
			if (SplashStart && !inTutorial && !shownSplash)
			{
				shownSplash = true;
				LevelNameSplashEvent.Trigger(SceneManager.GetActiveScene().name);
				if (!GameManager.Instance.Paused)
				{
					CorgiEngineEvent.Trigger(CorgiEngineEventTypes.StartSplash);
					ToggleCharacterPause();
		...
		protected virtual void LevelGUIStart()
		{
			...
			// set the number of enemies in the GUI
			EnemyCountEvent.Trigger(EnemyCount);

			
			// fade in
            if (Players.Count > 0)
            {
                MMFadeOutEvent.Trigger(IntroFadeDuration, FadeTween, FaderID, true, Players[0].transform.position);
            }
            else
            {
                MMFadeOutEvent.Trigger(IntroFadeDuration, FadeTween, FaderID, true, Vector3.zero);
            }
        }
	    ...
	    protected virtual IEnumerator GotoLevelCo(string levelName, bool fadeOut = true)
		{
			...
			CorgiEngineEvent.Trigger(CorgiEngineEventTypes.UnPauseSilent);
		...
		/// <summary>
        /// See if player is close enough to an object to activate things like feedbacks
        /// </summary>
        /// <param name="target"></param>
        /// <returns></returns>
		public virtual bool CheckPlayerProximity(Vector3 target)
        {
			...
			float distanceToTarget = Mathf.Sqrt((xDistance * xDistance) + (yDistance * yDistance));
		...
		{
			if (characterEvent.EventType == MMCharacterEventTypes.EnemyKill)
            {
				EnemyCount--;
				EnemyCountEvent.Trigger(EnemyCount);
				if (EnemyCount == 0)
                {
					ExitUnlockedEvent.Trigger(true);
                }
			}
		}

		public virtual void OnMMEvent(CloseTutorialEvent e)
		{
			inTutorial = false;
			saver.SetSeenTutorial(TutorialName);
			if (!SplashStart)
            {
				ToggleCharacterPause();
				CorgiEngineEvent.Trigger(CorgiEngineEventTypes.EndSplash);
			}
		}
		...
		protected virtual void OnEnable()
		{
			...
			this.MMEventStartListening<CloseTutorialEvent>();
		}
		...
		protected virtual void OnDisable()
		{
			...
			this.MMEventStopListening<CloseTutorialEvent>();
		}
	}
}
